      ...
      <div className="relative mx-auto grid max-w-7xl items-stretch gap-14 lg:grid-cols-[1.05fr_.95fr]">
        <motion.div initial="hidden" animate="visible" variants={stagger} className="flex h-full flex-col justify-between">
          <div>
            ...
            <motion.h1 variants={fadeUp} className="max-w-4xl text-5xl font-black leading-[0.95] tracking-[-0.05em] text-ink md:text-7xl">Operational chaos becomes expensive long before companies notice it.</motion.h1>
            <motion.p variants={fadeUp} className="mt-7 max-w-2xl text-lg leading-8 text-muted md:text-xl">DENOISE helps scaling companies identify operational failure points, restructure execution, and build systems that make growth predictable, measurable, and scalable.</motion.p>
          </div>
          <motion.div variants={fadeUp} className="mt-9 flex flex-col gap-4 sm:flex-row sm:items-end sm:gap-6">
            <CTAButton />
            <p className="max-w-[240px] pb-1 text-sm leading-6 text-muted-3">Built for companies where growth has outpaced operational clarity.</p>
          </motion.div>
        </motion.div>
